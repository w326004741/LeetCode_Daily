...
public class DecompressRLElist {
    ...
    public int[] decompressRLElist(int[] nums) {
        ...
        int size = 0;
        for (int i = 0; i < nums.length; i += 2) { // 确定新数组长度，因为新数组的长度是：nums[i] + num[i+2]. E.g. {1,2,3,4} = 1 + 3 = 4.
            size += nums[i];
        }
        ...
        for (int i = 0; i < nums.length; i += 2) {
            int freq = nums[i];  // the length of array
            int val = nums[i + 1]; // the element value of array
